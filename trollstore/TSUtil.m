// TSUtil.m — Minimal spawnRoot implementation extracted from opa334/TrollStore.
//
// Only spawnRoot is exposed; helper functions (fd_is_valid, getNSStringFromFile)
// are file-local. See TSUtil.h for rationale on the stripping.
//
// Behavior: posix_spawn with the persona-mgmt attributes set so the child runs
// as uid 0. On iOS 17.6+ persona-mgmt is mitigated and the spawn fails;
// PXRootHelper gates the OS version before calling here.
...
// persona-mgmt private spawn attrs — symbol exists in libsystem on iOS,
// no public header. Declared extern so we can link.
...
int spawnRoot(NSString *path, NSArray *args, NSString **stdOut, NSString **stdErr)
{
    ...
    if (stdOut || stdErr)
    {
        logQueue = dispatch_queue_create("com.hydra.projectx.LogCollector", NULL);
